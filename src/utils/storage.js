...
/**
 * 특정 날짜의 업무를 { [memberId]: [task, ...] } 형태로 반환
 */
export async function fetchDailyTasks(date) {
  ...
  for (const row of data ?? []) {
    const key = row.member_id || '_orphan'
...
/**
 * 기간 내 일일 업무를 { [date]: { [memberId]: [task, ...] } } 형태로 반환
 */
export async function fetchDailyTasksRange(from, to) {
  const { data, error } = await supabase
    .from('daily_tasks')
    .select('*')
    .gte('date', from)
...
/**
 * 업무 한 건 추가
 * @param {{ id, date, memberId, title?, done?, memo?, orderIndex? }} task
 */
export async function insertDailyTask({ id, date, memberId, title = '', done = false, memo = '', orderIndex = 0 }) {
...
/**
 * 업무 수정 (title / done / memo 중 변경된 필드만)
 */
export async function updateDailyTask(id, patch) {
  ...
  const { error } = await supabase.from('daily_tasks').update(row).eq('id', id)
  if (error) throw error
}
